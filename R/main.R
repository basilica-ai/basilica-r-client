...
createConnection <- function(
    auth_key = character(), server = character(), retries = numeric(),
  backoff_factor = numeric()) {
  result <- methods::new(
    "Connection", auth_key, server, retries, backoff_factor)
}
...
Connection <- setRefClass("Connection",
  fields = list(auth_key = "character", server = "character",
    retries = "numeric", backoff_factor = "numeric"),
  methods = list(
initialize = function(auth_key = character(), server = character(),
    retries = numeric(), backoff_factor = numeric()) {
...
embed_image = function(image = character(), model = "generic",
    version = "default", timeout = 5) {
  "Embed an image"
  response <- .self$embed_images(list(image), model = model, version = version,
    timeout = timeout)
  result <- response[[1]]
},
embed_images = function(images = character(), model = "generic",
    version = "default", timeout = 5) {
...
embed_sentence = function(sentence = character(),
    model = "english", version = "default", timeout = 5) {
  "Embed a single sentence"
  response <- .self$embed_sentences(list(sentence),
    model = model, version = version, timeout = timeout)
  result <- response[[1]]
},
embed_sentences = function(
    sentences = list(), model = "english", version = "default", timeout = 5) {
...
embed <- function(
    auth_key = character(), url = character(), data = list(), timeout = 5) {
  authorization <- paste("Bearer", auth_key)
  response <- httr::POST(
    url, body = list(data = data), encode = "json",
